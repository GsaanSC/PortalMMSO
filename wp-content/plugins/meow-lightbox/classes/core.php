...
class Meow_MWL_Core {
	...
	private function convert_cdn( $url ) {
		...
		if( $cdn_static_url ) {
			...
			if ( filter_var( $cdn_url, FILTER_VALIDATE_URL ) ) {
				...
				error_log( '[Meow Lightbox] The CDN URL is not valid: ' . $cdn_url );
			}
		}
		return $url;
	}
	...
	******************************************************************************/
	...
	function lightboxify( $buffer ) {
		...
		if( !$html ){
			error_log( '⚠️  | Meow Lightbox: HtmlDomParser wasn\'t used. Trying DiDom.' );
			$html = new Document();
		...
		if ( $this->isObMode ) {
			...
			if ( count( $matches ) < 2 ) {
				error_log( '🪲  | Meow Lightbox: Output Buffering was used on a non HTML response. Returning the current buffer.' );
				return $buffer;
			}
